...
<body>
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <p class="text-4xl font-extrabold text-primary mt-1">{{ $totalAlternatif ?? 0 }}</p>
            ...
            <p class="text-4xl font-extrabold text-primary mt-1">{{ $totalKriteria ?? 0 }}</p>
            ...
            <p class="text-4xl font-extrabold text-primary mt-1">{{ $totalDM ?? 0 }}</p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-lg card-shadow border-t-4 border-primary">
                        ...
                        <div class="space-y-4">
                            <p class="text-sm font-medium text-primary">{{ ($progressPersen ?? 0) }}% penilaian telah selesai.</p>
                            <!-- Progress Bar -->
                            <div class="w-full bg-gray-200 rounded-full h-2.5">
                                <div class="bg-primary h-2.5 rounded-full" style="width: {{ ($progressPersen ?? 0) }}%"></div>
                            </div>
                            <p class="text-xs text-gray-500">{{ ($progressPersen ?? 0) }}% dari decision maker telah menyelesaikan penilaian.</p>
                        </div>
                    </div>
                            ...
                            @if(!empty($recentActivities) && count($recentActivities) > 0)
                                @foreach($recentActivities as $act)
                                    <li class="flex justify-between items-center border-b border-gray-100 pb-2">
                                        <span>{{ $act->user->username ?? 'Unknown' }} menilai alternatif</span>
                                        <span class="text-xs text-gray-400">{{ isset($act->updated_at) ? $act->updated_at->diffForHumans() : '' }}</span>
                                    </li>
                                @endforeach
                            @else
                                <li class="flex justify-between items-center border-b border-gray-100 pb-2">
                                    <span>Belum ada aktivitas penilaian.</span>
