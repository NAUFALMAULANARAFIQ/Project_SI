                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead>
                                <tr class="table-header">
                                    ...
                                    <th class="px-6 py-3 text-center text-xs font-semibold text-primary uppercase tracking-wider">Skor Borda</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @if(isset($msg))
                                    <tr>
                                        <td colspan="4" class="px-6 py-4 text-center text-gray-500">{{ $msg }}</td>
                                    </tr>
                                @elseif(!empty($hasilBorda) && count($hasilBorda) > 0)
                                    @foreach($hasilBorda as $i => $hb)
                                        <tr class="{{ $i == 0 ? 'bg-green-100/50' : ($i % 2 ? '' : 'bg-gray-50') }}">
                                            <td class="px-6 py-4 whitespace-nowrap text-center {{ $i==0? 'text-2xl font-extrabold text-primary':'text-lg' }}">#{{ $i + 1 }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-lg font-{{ $i==0? 'bold text-primary' : 'medium text-gray-800' }}">{{ $hb['nama_mp'] ?? $hb['kode_mp'] ?? 'Unknown' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-800">{{ number_format($hb['vi'] ?? 0, 5) }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-800">{{ $hb['poin'] ?? 0 }}</td>
                                        </tr>
                                    @endforeach
                                @elseif(!empty($hasilAkhir) && $hasilAkhir->count() > 0)
                                    @foreach($hasilAkhir as $index => $row)
                                        <tr class="{{ $index == 0 ? 'bg-green-100/50' : ($index % 2 ? '' : 'bg-gray-50') }}">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-800">-</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                        <td colspan="4" class="px-6 py-4 text-center text-gray-500">Belum ada data hasil perhitungan.</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
