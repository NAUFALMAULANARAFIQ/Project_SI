...
<body>
    ...
    <script>
    // --- DATA DARI SERVER ---
    // Controller PerhitunganController::create() sudah mengirimkan
    // $mataKuliah dan $kriterias ke view mahasiswa.perhitungan_form,
    // route admin.penilaian menggunakan view yang sama, jadi gunakan data tersebut.
    const kriteriaData = @json($kriterias ?? []);
    const matakuliahData = @json($mataKuliah ?? []);
    const decisionMaker = @json(Session::get('user_session')->username ?? 'DM');
        ...
        function renderKriteriaTable() {
            kriteriaPenilaianBody.innerHTML = '';
                ...
                // Normalize model fields: nama_kriteria, cost_benefit
                const kode = kriteria.kode_kriteria ?? kriteria.kode ?? ('C' + (index+1));
                const nama = kriteria.nama_kriteria ?? kriteria.nama ?? '';
                const sifat = (kriteria.cost_benefit ?? kriteria.sifat ?? '').toString();
                const isCost = sifat.toLowerCase() === 'cost';
                ...
                const row = `
                    <tr class="${index % 2 === 1 ? 'bg-gray-50' : 'bg-white'}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${kode}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${nama} (${kode})</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm ${sifatClass} font-semibold">${sifat}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <select name="${kode}" class="p-2 border border-gray-300 rounded-lg text-sm select-custom w-full max-w-[100px] mx-auto" required>
                                ${selectOptions}
                            </select>
                        </td>
                    </tr>
