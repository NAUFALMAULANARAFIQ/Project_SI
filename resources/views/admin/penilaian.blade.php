    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto bg-[#fdfdfd]">
            <header class="bg-white shadow-sm h-16 flex items-center justify-between px-6 sticky top-0 z-10 border-b border-gray-100">
                <h2 class="text-xl font-semibold text-primary">Form Penilaian Matakuliah</h2>
            ...
            <div class="p-8">
                ...
                <div class="bg-white p-8 rounded-xl card-shadow border-t-4 border-secondary">
                    ...
                    <form action="{{ route('admin.penilaian.index') }}" method="GET" class="mb-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center border-b pb-4">

                            <div>
                                <label for="semester" class="block text-sm font-semibold text-gray-700 mb-1">Pilih Semester:</label>
                                <select name="semester" onchange="this.form.submit()" class="w-full p-3 border-2 border-gray-300 rounded-lg select-custom focus:border-primary focus:ring-primary">
                                    <option value="" disabled {{ !$selectedSemester ? 'selected' : '' }}>-- Pilih Semester --</option>
                                    @foreach([4,5,6,7,8] as $sem)
                                        <option value="{{ $sem }}" {{ $selectedSemester == $sem ? 'selected' : '' }}>Semester {{ $sem }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div>
                                <label for="mk_id" class="block text-sm font-semibold text-gray-700 mb-1">Pilih Matakuliah:</label>
                                <select name="mk_id" onchange="this.form.submit()" class="w-full p-3 border-2 border-gray-300 rounded-lg select-custom focus:border-primary focus:ring-primary" {{ empty($matakuliahs) ? 'disabled' : '' }}>
                                    <option value="" disabled {{ !$selectedMkId ? 'selected' : '' }}>-- Pilih Matakuliah --</option>
                                    @foreach($matakuliahs as $mk)
                                        <option value="{{ $mk->id_mp ?? $mk->id }}" {{ $selectedMkId == ($mk->id_mp ?? $mk->id) ? 'selected' : '' }}>
                                            {{ $mk->nama_mp }} ({{ $mk->kode_mp }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    @if($currentMk && count($kriterias) > 0)
                    <form action="{{ route('admin.penilaian.store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="mk_id" value="{{ $selectedMkId }}">
                        <input type="hidden" name="semester_hidden" value="{{ $selectedSemester }}">

                        <div class="pt-2">
                            <div class="flex justify-between items-end mb-4">
                                <h2 class="text-xl font-bold text-secondary">
                                    Penilaian: {{ $currentMk->nama_mp }}
                                </h2>
                                <span class="text-sm text-gray-500">Skala 1 (Sangat Rendah) - 5 (Sangat Tinggi)</span>
                            </div>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead>
                                        <tr class="table-header">
                                            ...
                                            <th class="px-6 py-3 text-left text-xs font-semibold text-primary uppercase">Kriteria</th>
                                            <th class="px-6 py-3 text-left text-xs font-semibold text-primary uppercase">Sifat</th>
                                            <th class="px-6 py-3 text-center text-xs font-semibold text-primary uppercase">Nilai</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 text-sm font-medium text-gray-800">{{ $k->kode_kriteria }}</td>
                                            <td class="px-6 py-4 text-sm text-gray-800">{{ $k->nama_kriteria }}</td>
                                            <td class="px-6 py-4 text-sm font-semibold {{ strtolower($k->sifat) == 'cost' ? 'text-red-500' : 'text-green-600' }}">
                                                {{ $k->sifat }}
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                <select name="nilai[{{ $k->id_kriteria ?? $k->id }}]" required
                                                    class="p-2 border border-gray-300 rounded-lg text-sm select-custom w-full max-w-[100px] mx-auto focus:border-primary focus:ring-primary">
                            ...
                            <div class="flex justify-end space-x-4 pt-6">
                                <a href="{{ route('admin.penilaian.index') }}" class="px-4 py-3 bg-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-400 transition">Batal</a>
                                <button type="submit" class="px-6 py-3 bg-primary text-white font-semibold rounded-lg hover:bg-[#5d6b59] transition shadow-lg">
                                    Simpan Penilaian
                                </button>
                            </div>
                        </div>
                    ...
                    @elseif($selectedMkId)
                        <div class="mt-4 p-4 bg-yellow-50 text-yellow-700 rounded-lg border border-yellow-200">
