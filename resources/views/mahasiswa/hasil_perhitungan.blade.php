...
<body>
    ...
    <script>
        async function loadResults() {
            try {
                const res = await window.axios.get('/api/perhitungan');
                const data = Array.isArray(res.data) ? res.data : [];
                const tbody = document.querySelector('table tbody');
                if (!tbody) return;
                if (data.length === 0) return; // keep server-rendered fallback
                tbody.innerHTML = '';
                // Aggregate by id_mp and sort by hasil descending
                const grouped = {};
                data.forEach(item => {
                    const id = item.id_mp || (item.mkPlhn && item.mkPlhn.id_mp) || null;
                    if (!id) return;
                    if (!grouped[id]) grouped[id] = { item, vals: [] };
                    grouped[id].vals.push(item);
                });
                const rows = Object.values(grouped).map(g => g.item).sort((a,b) => (b.hasil||0) - (a.hasil||0));
                rows.forEach((row, idx) => {
                    const tr = document.createElement('tr');
                    tr.className = idx==0 ? 'bg-green-100/50' : (idx%2? '':'bg-gray-50');
                    tr.innerHTML = `
                        <td class="px-6 py-4 whitespace-nowrap text-center ${idx==0? 'text-2xl font-extrabold text-primary':'text-lg'}">#${idx+1}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-lg font-${idx==0? 'bold text-primary' : 'medium text-gray-800'}">${(row.mkPlhn && row.mkPlhn.nama_mp) || row.id_mp || 'Unknown'}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-800">${Number(row.hasil||0).toFixed(5)}</td>
                    `;
                    tbody.appendChild(tr);
                });
            } catch (err) {
                console.error('Gagal memuat hasil perhitungan', err);
            }
        }
