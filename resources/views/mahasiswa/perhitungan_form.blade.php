...
<body>
        ...
        <main class="flex-1 overflow-y-auto">
            ...
            <div class="p-8">
                <div class="bg-white p-6 rounded-xl shadow-lg border-t-4 border-secondary">
                    ...
                    <form id="mahasiswa-perhitungan-form" class="space-y-8">
                        ...
                        <div class="flex justify-end space-x-4 pt-4">
                            <button id="submit-penilaian" type="button"
    ...
    <script>
        ...
        async function collectAndSubmit() {
            // Build payloads: for each mata kuliah, collect nilai for each kriteria
            const rows = Array.from(document.querySelectorAll('tbody tr'));
            // We'll send one perhitungan per selected mata kuliah row where user provided values
            const promises = [];

            rows.forEach(row => {
                const mkIdCell = row.querySelector('td');
                if (!mkIdCell) return;
                // extract mata kuliah id from select name attributes present in selects
                const selects = row.querySelectorAll('select');
                if (!selects || selects.length === 0) return;

                // collect nilai data for this mk
                const details = [];
                selects.forEach(s => {
                    // name like nilai[<id_mp>][<id_kriteria>]
                    const m = s.name.match(/nilai\[(\d+)\]\[(\d+)\]/);
                    if (m) {
                        const id_mp = m[1];
                        const id_kriteria = m[2];
                        const val = Number(s.value) || 0;
                        details.push({ id_kriteria: id_kriteria, nilai: val });
                    }
                });

                // get id_mp from first select name
                const firstSelect = selects[0];
                const match = firstSelect.name.match(/nilai\[(\d+)\]\[(\d+)\]/);
                if (!match) return;
                const id_mp = match[1];

                // id_user from server-side session (if available)
                const id_user = '{{ session("user_session")->id ?? "" }}';

                // push axios post
                promises.push(window.axios.post('/api/perhitungan', { id_user: id_user, id_mp: id_mp, hasil: null, details }));
            });
            ...
            try {
                ...
                alert('Gagal menyimpan penilaian. Cek konsol untuk detail.');
            }
        }

        document.getElementById('submit-penilaian').addEventListener('click', collectAndSubmit);
    </script>
</body>
</html>
